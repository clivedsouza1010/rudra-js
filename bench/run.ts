...
function table(results: readonly ArmResult[]): string {
  const lines: string[] = [];
  lines.push(
    '| Arm | Views | Model calls | Cache hits | Cost / 1k views | Median ms | p95 | p99 |',
  );
  lines.push('| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |');
  for (const result of results) {
    ...
    lines.push(
      `| ${result.arm} | ${result.views} | ${result.modelCalls} | ${hits} | ${cost} | ` +
...
async function main(): Promise<void> {
  ...
  const arms: ArmSpec[] = [
    { name: 'b deterministic', options: { provider: null }, rule: { fallback: 'all' } },
    {
      name: 'c cohort',
      options: { provider: createStubProvider(RECORDED_USAGE), generation: 'cohort' },
      rule: { fallback: 'none', minCacheHitRate: 0.5 },
    },
    {
      name: 'd per-shopper',
      options: { provider: createStubProvider(RECORDED_USAGE), generation: 'per-shopper' },
